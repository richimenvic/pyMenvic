...
TOOL_VERSION = "MVP 0.4.7"
TOOL_BUILD_DATE = "2026-05-06"
TOOL_TITLE_SEPARATOR = u" \u2014 "


def get_tool_dialog_title():
    return u"{}{}{}".format(__title__, TOOL_TITLE_SEPARATOR, TOOL_VERSION)
...
from System.Windows import Visibility, FontWeights, Style, Setter, DataTrigger, Window, WindowStartupLocation, Thickness, HorizontalAlignment, VerticalAlignment, ResizeMode, GridLength, TextWrapping
from System.Windows.Controls import DataGridCell, StackPanel, TextBlock, Button, Grid, RowDefinition
...
def is_scale_import_field(field_name, metadata):
    normalized_name = normalize_text(field_name).lower()
    if normalized_name in ("view scale", "scale"):
        return True

    if metadata:
        normalized_metadata_name = normalize_text(metadata.get("Name", "")).lower()
        if normalized_metadata_name in ("view scale", "scale"):
            return True

    return False


def try_parse_import_float(value):
...
def normalize_import_ratio_text(value):
    text = normalize_text(value)
    if ":" not in text:
        return ""
    ...
    try:
        left = safe_text(int(float(left.replace(",", "."))))
        right = safe_text(int(float(right.replace(",", "."))))
    except Exception:
        pass
...
def get_import_compare_result(current_value, import_value, field_name="", metadata=None, param=None):
    current_text = normalize_text(current_value)
    import_text = normalize_text(import_value)

    if current_text == import_text:
        return True, True

    current_ratio = normalize_import_ratio_text(current_text)
    import_ratio = normalize_import_ratio_text(import_text)
    if current_ratio or import_ratio:
        if current_ratio and import_ratio:
            return True, current_ratio == import_ratio
        return False, False

    current_number = try_parse_import_float(current_text)
    import_number = try_parse_import_float(import_text)
    if current_number is not None or import_number is not None:
        if current_number is not None and import_number is not None:
            return True, abs(current_number - import_number) < 0.000001
        if is_scale_import_field(field_name, metadata):
            return False, False

        try:
            if param is not None and param.StorageType in (DB.StorageType.Double, DB.StorageType.Integer):
                return False, False
        except Exception:
            pass
...
def values_are_effectively_equal(current_value, excel_value, field_name="", metadata=None, param=None):
    safe_to_compare, values_match = get_import_compare_result(
        current_value,
        excel_value,
        field_name,
        metadata,
...
def apply_parameter_value_and_verify(param, excel_value, field_name, metadata):
    if param is None:
        return False, "", ""

    current_value = get_parameter_preview_value(param)
    if not set_parameter_value(param, excel_value):
        return False, current_value, current_value
    ...
    if values_match:
        return True, current_value, refreshed_value

    return False, current_value, refreshed_value


def make_unique_table_column_name(table, base_name):
...
def build_import_preview_context(data_info, include_markers=True):
    table = DataTable()
    ...
    for col_info in editable_columns:
        ...
        column_name = make_unique_table_column_name(table, base_name)
            ...
            if len(unresolved_lines) < 10:
                unresolved_lines.append(
                    "Row {} | UniqueId='{}' | ElementId='{}'".format(
                        excel_row,
                        safe_text(values.get(1, "")),
                        safe_text(values.get(2, ""))
                    )
                )
    ...
    return {
        ...
        "row_excel_rows": row_excel_rows,
        "column_map": column_map,
        "original_values": original_values,
        "changed_marker_map": changed_marker_map,
        "unresolved": unresolved_count,
        "unsafe": skipped_unsafe,
        "unresolved_lines": unresolved_lines,
        "editable_count": len(preview_columns)
    }


def apply_preview_context_to_model(table, row_elements, column_map, original_values, row_excel_rows=None):
    updated = 0
    unchanged = 0
    locked = 0
    missing = 0
    ...
    for row_index in range(table.Rows.Count):
        ...
        for column_name, column_info in column_map.items():
            try:
                ...
                ok, old_param_value, refreshed_value = apply_parameter_value_and_verify(
                    param,
                    new_value,
                    field_name,
                    metadata
                ...
                if ok:
                    ...
                    if len(failed_lines) < 15:
                        ...
                        try:
                            if row_excel_rows is not None and row_index < len(row_excel_rows):
                                excel_row = row_excel_rows[row_index]
                        except Exception:
                            pass
                        failed_lines.append(
                            "Row {} | Id {} | {} | write not verified | expected '{}' | got '{}'".format(
                                excel_row,
...
def apply_import_data_info_to_model(data_info):
    context = build_import_preview_context(data_info, include_markers=False)
    ...
    result = {
        ...
        "failed": 0,
        "duplicates": 0,
        "unsafe": context.get("unsafe", 0),
        "unresolved": context.get("unresolved", 0),
        "failed_lines": [],
        "duplicate_lines": [],
        "unresolved_lines": context.get("unresolved_lines", [])
    }
    ...
    try:
        t.Start()
        started = True
        applied = apply_preview_context_to_model(
            context.get("table"),
            context.get("row_elements"),
            context.get("column_map"),
            context.get("original_values"),
            context.get("row_excel_rows")
        )
        t.Commit()
        started = False

        for key in ("updated", "unchanged", "locked", "missing", "failed", "duplicates", "failed_lines", "duplicate_lines"):
            result[key] = applied.get(key, result.get(key))
...
def add_import_result_line(panel, label, value):
    text = TextBlock()
    text.Text = "{}: {}".format(label, value)
    text.FontSize = 14
    text.Foreground = BrushConverter().ConvertFrom("#5A6472")
    text.Margin = Thickness(0, 4, 0, 0)
    panel.Children.Add(text)


def show_import_result_dialog(title_text, updated, unchanged, skipped_unsafe, failed, duplicate_count, unresolved_count, missing_param_count, locked=0, fallback_message=""):
    try:
        has_errors = failed or duplicate_count or unresolved_count or missing_param_count

        window = Window()
        window.Title = u"SheetLink - pyMENVIC \u2014 {}".format(TOOL_VERSION)
        window.Width = 560
        window.Height = 320
        window.MinWidth = 540
        ...
        window.Background = BrushConverter().ConvertFrom("#F6F8FA")

        layout = Grid()
        layout.Margin = Thickness(0)
        content_row = RowDefinition()
        footer_row = RowDefinition()
        footer_row.Height = GridLength(70)
        layout.RowDefinitions.Add(content_row)
        layout.RowDefinitions.Add(footer_row)
        window.Content = layout

        root = StackPanel()
        root.HorizontalAlignment = HorizontalAlignment.Stretch
        root.Margin = Thickness(34, 30, 34, 8)
        Grid.SetRow(root, 0)
        layout.Children.Add(root)

        header = TextBlock()
        if has_errors:
            header.Text = "Import completed with warnings."
            header.Foreground = BrushConverter().ConvertFrom("#B26A00")
        else:
            header.Text = "Import completed successfully."
            header.Foreground = BrushConverter().ConvertFrom("#167C80")
        header.TextWrapping = TextWrapping.NoWrap
        header.FontSize = 26
        header.FontWeight = FontWeights.SemiBold
        header.Margin = Thickness(0, 0, 0, 22)
        root.Children.Add(header)

        summary = TextBlock()
        if updated:
            summary.Text = "Updated values: {}".format(updated)
        else:
            summary.Text = "No model changes were needed."
        summary.FontSize = 16
        summary.FontWeight = FontWeights.SemiBold
        summary.Foreground = BrushConverter().ConvertFrom("#151A21")
        summary.Margin = Thickness(0, 0, 0, 26)
        root.Children.Add(summary)

        details_needed = unchanged or skipped_unsafe or locked or has_errors
        if details_needed:
            details_header = TextBlock()
            details_header.Text = "Details:"
            details_header.FontSize = 14
            details_header.FontWeight = FontWeights.SemiBold
            details_header.Foreground = BrushConverter().ConvertFrom("#5A6472")
            details_header.Margin = Thickness(0, 0, 0, 10)
            root.Children.Add(details_header)

            if unchanged:
                add_import_result_line(root, "Unchanged cells", unchanged)
            if skipped_unsafe:
                add_import_result_line(root, "Skipped format-protected values", skipped_unsafe)
            if locked:
                add_import_result_line(root, "Read-only skipped", locked)
            if failed:
                add_import_result_line(root, "Failed", failed)
            if duplicate_count:
                add_import_result_line(root, "Duplicate conflicts", duplicate_count)
            if unresolved_count:
                add_import_result_line(root, "Unresolved rows", unresolved_count)
            if missing_param_count:
                add_import_result_line(root, "Missing parameters", missing_param_count)

        close_button = Button()
        close_button.Content = "Close"
        close_button.Width = 125
        close_button.Height = 38
        close_button.Margin = Thickness(0, 0, 34, 12)
        close_button.HorizontalAlignment = HorizontalAlignment.Right
        close_button.VerticalAlignment = VerticalAlignment.Center
        close_button.HorizontalContentAlignment = HorizontalAlignment.Center
        close_button.VerticalContentAlignment = VerticalAlignment.Center
        close_button.Background = BrushConverter().ConvertFrom("#167C80")
        close_button.Foreground = Brushes.White
        close_button.BorderThickness = Thickness(1)
        close_button.FontWeight = FontWeights.SemiBold
        close_button.Click += lambda sender, args: window.Close()
        Grid.SetRow(close_button, 1)
        layout.Children.Add(close_button)

        window.ShowDialog()

    except Exception:
        TaskDialog.Show(title_text, fallback_message)


def run_import_apply(xlsx_path, progress_callback=None):
    excel_app = None
    workbooks = None
    workbook = None
    ...
    try:
        ...
        result = apply_import_data_info_to_model(data_info)

        updated = result.get("updated", 0)
        unchanged = result.get("unchanged", 0)
        locked = result.get("locked", 0)
        skipped_unsafe = result.get("unsafe", 0)
        failed = result.get("failed", 0)
        duplicate_count = result.get("duplicates", 0)
        unresolved_count = result.get("unresolved", 0)
        missing_param_count = result.get("missing", 0)
        duplicate_lines = result.get("duplicate_lines", [])
        unresolved_lines = result.get("unresolved_lines", [])
        ...
        if not has_errors:
            if updated:
                ...
                message.append("Version: {}".format(TOOL_VERSION))
                message.append("")
                ...
                message.append("Version: {}".format(TOOL_VERSION))
                message.append("")
                message.append("No model changes were needed.")

            if unchanged or skipped_unsafe or locked:
                message.append("")
                message.append("Details:")
                if unchanged:
                    message.append("Unchanged cells: {}".format(unchanged))
                if skipped_unsafe:
                    message.append("Skipped format-protected values: {}".format(skipped_unsafe))
                if locked:
        ...
        TaskDialog.Show(
            get_tool_dialog_title(),
            "Import failed.\nVersion: {}\n\n{}".format(TOOL_VERSION, safe_text(ex))
        )
...
def run_import_preview(xlsx_path):
    ...
    try:
        ...
        message.append("Skipped unsafe formatted values: {}".format(skipped_unsafe))
        message.append("Missing parameters: {}".format(missing_param_count))
...
class ScheduleBrowserWindow(forms.WPFWindow):
    def __init__(self, xaml_path):
        forms.WPFWindow.__init__(self, xaml_path)
        try:
            self.Title = get_tool_dialog_title()
        except Exception:
            pass
        try:
            self.lblHeaderTitle.Text = "SheetLink - {}".format(TOOL_VERSION)
        except Exception:
            pass
    ...
    def _set_progress(self, value, message=None):
        ...
        try:
            self.lblHeaderProgress.Text = "{} | {}   {}%".format(label, TOOL_VERSION, progress_value)
        except Exception:
            pass
    ...
    def _build_preview_table_from_import_data(self, data_info):
        context = build_import_preview_context(data_info, include_markers=True)
        table = context.get("table")
        column_map = context.get("column_map") or {}

        self.preview_row_elements = context.get("row_elements") or []
        self.preview_column_map = column_map
        self.preview_original_values = context.get("original_values") or {}
        self.preview_changed_marker_map = context.get("changed_marker_map") or {}
        self.preview_import_row_count = table.Rows.Count if table is not None else 0
        self.preview_import_editable_count = context.get("editable_count", 0)
        ...
        try:
            ...
            if show_dialog:
                if only_noop:
                    ...
                    TaskDialog.Show(get_tool_dialog_title(), "\n".join(message))
    ...
    def on_import_from_excel_clicked(self, sender, args):
        ...
        try:
            ...
            run_import_apply(xlsx_path, self._set_progress)
