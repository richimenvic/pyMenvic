...
class TableEntry(INotifyPropertyChanged):
    def __init__(
        ...
        self._worksheet_options = worksheet_options if worksheet_options else []
        self._region = region
    ...
    def _set(self, private_name, prop_name, value):
        if getattr(self, private_name) != value:
            ...
            if prop_name == "Status":
                if value != "Skipped":
                    self._status_detail = ""
                    self._notify("StatusDetail")
                self._notify("StatusIcon")
                self._notify("StatusToolTip")
                self._notify("FilePathDisplay")
                self._notify("FilePathToolTip")
    ...
    def StatusIcon(self):
        ...
        if status in ("Updated", "Ready to Update", "Ready"):
            return u"✓"
        if status == "Updating...":
            return u"↻"
        if status in ("Failed", "Missing File", "Invalid Region"):
            return u"!"
        if status == "Skipped":
            return u"–"
        return u""
    ...
    @StatusDetail.setter
    def StatusDetail(self, value):
        self._set("_status_detail", "StatusDetail", value or "")
        self._notify("StatusToolTip")
    ...
    @property
    def WorksheetOptions(self):
        return self._worksheet_options

    @WorksheetOptions.setter
    def WorksheetOptions(self, value):
        if value is None:
            value = []
        self._worksheet_options = value
        self._notify("WorksheetOptions")
    ...
    @property
    def FilePathToolTip(self):
        path = self._file_path or ""
        if not path:
            return ""
        if self._status == "Missing File":
