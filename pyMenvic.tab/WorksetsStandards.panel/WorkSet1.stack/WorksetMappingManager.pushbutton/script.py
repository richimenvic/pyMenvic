...
class WorksetMappingManagerWindow(forms.WPFWindow):
    ...
    def ensure_missing_target_worksets(self, rows_to_map):
        ...
        for row in rows_to_map:
            target_name = (row.FinalTarget or "").strip()
            row.FinalTarget = target_name
    ...
    def setup_grid(self):
        ...
        try:
            self.dgMappings.BeginningEdit += self.on_grid_beginning_edit
            self.dgMappings.PreviewMouseLeftButtonDown += self.on_grid_preview_mouse_left_button_down
        except Exception:
            pass
    ...
    def _find_visual_parent(self, child, parent_type):
        try:
    ...
    def on_grid_preview_mouse_left_button_down(self, sender, args):
        try:
            dep = args.OriginalSource
            cell = self._find_visual_parent(dep, DataGridCell)
            if cell is None:
                return
            if cell.IsEditing:
                return
            if not cell.IsFocused:
                cell.Focus()
            parent_grid = self.dgMappings
            if parent_grid.CurrentCell != parent_grid.CurrentCell:
                pass
            parent_grid.BeginEdit(args)
        except Exception:
            pass
    ...
    def on_grid_beginning_edit(self, sender, args):
        try:
            row = args.Row.Item
            col = args.Column
            header = str(col.Header) if col and col.Header is not None else ""
            if header == "FINAL TARGET" and is_target_empty(row.FinalTarget) and row.SuggestedTarget:
    ...
    def refresh_target_counts(self):
        ...
        current_targets = [PLACEHOLDER_TARGET] + sorted(self._workset_map.keys())

        for row in self._all_rows:
            current_value = (row.FinalTarget or "").strip()
            if current_value:
                row.FinalTarget = current_value

            row.AvailableTargets.Clear()
            for item in current_targets:
                row.AvailableTargets.Add(item)
            if current_value and current_value not in current_targets:
                row.AvailableTargets.Add(current_value)
    ...
    def on_apply(self, sender, args):
        ...
        same_target = [r for r in rows_to_map if normalize_name(r.SourceName) == normalize_name((r.FinalTarget or "").strip())]
        if same_target:
            MessageBox.Show(
