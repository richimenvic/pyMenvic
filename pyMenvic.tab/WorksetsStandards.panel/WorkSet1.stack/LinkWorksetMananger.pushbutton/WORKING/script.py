...
def infer_link_workset(name):
    try:
        ...
        if any(t in tokens for t in ["MEC", "ME", "MECH", "MECHANICAL"]):
            return "LINK_MECH", "MECH", False

        if any(t in tokens for t in ["STR", "SRT", "ST"]):
            return "LINK_STR", "STR", False

        if "PLM" in tokens:
            return "LINK_PLM", "PLM", False

        if any(t in tokens for t in ["ELE", "EL"]):
            return "LINK_ELE", "ELE", False

        if any(t in tokens for t in ["ARC", "AR"]):
...
def get_safe_default_workset_name(preferred_name, workset_names):
    preferred_name = safe_str(preferred_name).strip()

    if preferred_name and preferred_name in workset_names:
        return preferred_name

    if "LINK_REF" in workset_names:
        return "LINK_REF"
...
def row_has_pending_change(row):
    try:
        target_name = safe_str(row.SelectedTargetWorksetName).strip()
...
class LinkWorksetManagerWindow(forms.WPFWindow):
    ...
    def load_data(self):
        ...
        for ws in self.worksets:
            ws_name = safe_str(ws.Name)
            workset_names.append(ws_name)
    ...
    def on_apply(self, sender, args):
        ...
        try:
            for row in changed_rows:
                ...
                if not target_ws:
                    errors.append("Target workset not found for '{}'.".format(row.LinkName))
                    continue
