...
def arrow_code_from_name(a_name):
    ...
    def _unit_suffix_no_underscore():
        # Para DF/DO/BX/ET/HE/DG/TRF en mm: sin guion bajo (ej DF1.5, BX2.25)
        if frac:
            return "_" + frac
        if mmv:
            return _mm_trim(mmv)
    ...
    def _unit_suffix_with_underscore():
        # Para AO90 en mm: con guion bajo (ej AO90_1.5)
        if frac:
            return "_" + frac
        if mmv:
            return "_" + _mm_trim(mmv)
        return ""

    is_mep = ln.startswith("mep") or ("mep -" in ln)

    # Arrow Filled (robusto): toma el primer número (15/20/30...) y arma AFxx
    if "arrow filled" in ln:
        m = re.search(r'(\d+)', ln)
        code = "AF{}".format(m.group(1)) if m else "AF"
        return ("MEP_" + code) if is_mep else code

    # Arrow Open 90 (mm o fracción).
    # Formato requerido:
    #   mm   -> AO90_1.25mm
    #   frac -> AO90_1/16"
    ...
    # Box Filled (y orden invertido Filled Box)
...
if office_skip_log and (created_standard or count_renamed):
    output.print_md("## Office standards verificados (existían y estaban limpios)")
