...
def apply_props(sub, weight, color_obj, pattern_id, gtype=DB.GraphicsStyleType.Projection):
    ...
    # Pattern: solo si es LinePatternElement valido (evita errores con InvalidElementId)
...
def seed_canonical_standards(line_cat, gtype, protected_names):
    """
    Crea canónicos base por weight (1..8):
      - RED
      - BLUE
      - (Hidden)  [sin color en el nombre]
    Se crean aunque no existan estilos equivalentes en el modelo.
    """
    ...
    for w in sorted(WEIGHT_LABELS.keys()):
        for color_label, color_obj, pattern_label, pattern_id in seeds:
            ...
            canon_sub = ensure_subcategory(line_cat, canon_name)
            if not canon_sub:
                continue
...
ANALYZE_ONLY = (choice == "ANALYZE ONLY (NO CHANGES)")
...
# ---------------- ANALYZE ONLY ----------------
if ANALYZE_ONLY:
    # Agrupar por propiedades para detectar duplicados
    groups = {}
    for sub in list(line_cat.SubCategories):
        ...
        if not k:
            continue
        groups.setdefault((k, cname, pname), []).append(sub)

    dup_groups = [(kcp, subs) for (kcp, subs) in groups.items() if len(subs) > 1]
    dup_groups.sort(key=lambda x: len(x[1]), reverse=True)

    output.print_md("# MENVIC | LINE MASTER")
    output.print_md("## ANALYSIS REPORT (NO CHANGES)")
    output.print_md("---")
    output.print_md("TOTAL LINE STYLES: {0}".format(len(list(line_cat.SubCategories))))
    output.print_md("TOTAL CURVE ELEMENTS: {0}".format(sum(usage_before.values())))
    output.print_md("DUPLICATE GROUPS (BY PROPERTIES): {0}".format(len(dup_groups)))
    output.print_md("---")

    output.print_md("### TOP DUPLICATE GROUPS")
    shown = 0
    for (kcp, subs) in dup_groups:
        if shown >= 12:
            break
        (k, cname, pname) = kcp
        w, color_key, pattern_key = k
        canon = canonical_name(w, cname, pname)

        output.print_md("**CANONICAL:** {0}  |  **COUNT:** {1}".format(canon, len(subs)))
        # listar hasta 10 items
        lim = 10
        i = 0
        for s in subs:
            if i >= lim:
            ...
            cnt = usage_before.get(gsid, 0) if gsid is not None else 0
            output.print_md("- {0}  (USE: {1})".format(s.Name, cnt))
            i += 1
        if len(subs) > lim:
            output.print_md("- ...")
        output.print_md("")
...
protected_names = set()     # nombres canonicos (no borrar)
...
with DB.Transaction(doc, "MENVIC: Line Master (Property Based)") as t:
    ...
    c0, u0 = seed_canonical_standards(line_cat, gtype, protected_names)
    created += c0
    forced += u0
    ...
    # 2) asegurar canonicos y mapear duplicados
    for (k, cname, pname), subs in groups.items():
        w, color_key, pattern_key = k
        ...
        # color "real" del canonico
        try:
            if color_key[0] == "RGB":
                rr, gg, bb = color_key[1]
                canon_color = DB.Color(rr, gg, bb)
            else:
                canon_color = subs[0].LineColor
        except:
            canon_color = subs[0].LineColor

        # patron desde el primero del grupo
        ...
        # mapear todos los del grupo que no sean el canonico
        for s in subs:
            try:
...
with DB.Transaction(doc, "MENVIC: Purge Line Styles") as t2:
    ...
    # 4.1) borrar duplicados mapeados
    ...
    # 4.2) borrar lo que quede sin uso (CurveElement) y no sea canonico
    for sub in list(line_cat.SubCategories):
        try:
            if not sub:
                continue
...
for sub in list(line_cat.SubCategories):
    try:
        ...
        if cnt > 0:
            ...
            rows.append((weight, sub.Name, cnt))  # (WEIGHT, NAME, COUNT)

    except:
        pass

# Orden: primero por weight, luego por nombre
