# -*- coding: utf-8 -*-
__title__  = "Estandarizar Textos (v2.4.8 - Office Arial MM)"
__author__ = "pyMenvic"

"""
POLITICA (confirmada):
- (T) significa Background = Transparent
- En este Revit: TEXT_BACKGROUND 1 => Transparent, 0 => Opaque
- STANDARD: SOLO mm, SOLO OPACO, SOLO Arial, tamaños:
  1.80, 2.00, 2.20, 2.50, 3.00, 3.50, 4.00, 4.50, 5.00, 5.50, 6.00, 7.00, 8.00
- Detectar tamaños "imperiales" comunes y mantener fracción en el nombre: "12.70mm (1/2")"
- NO tocar BuiltInParameter.LEADER_ARROWHEAD (solo lectura)
"""
...
def _extract_mm(arrow_name):
    m = re.search(r'(\\d+(?:\\.\\d+)?)\\s*mm', (arrow_name or "").lower())
...
def arrow_code_from_name(a_name):
    ...
    def _unit_suffix():
        if frac:
            return "_" + frac
        if mmv:
            return str(mmv).rstrip("0").rstrip(".")
        return ""

    is_mep = ln.startswith("mep") or ("mep -" in ln)

    # ==============================
    # ROBUST ARROW FILLED DETECTION
    # ==============================
    if "arrow filled" in ln:
        # Extrae el primer número que encuentre (15, 20, 30...)
        m = re.search(r'(\d+)', ln)
        if m:
            code = "AF{}".format(m.group(1))
        else:
            code = "AF"
    ...
    if "arrow open" in ln and "90" in ln:
        suf = _unit_suffix()
        return "AO90{}".format(suf)

    # Arrow simple con grado
...
try:
    ...
    INSTALLED_FONTS = set([f.Name for f in _ifc.Families])
except:
    INSTALLED_FONTS = set()
    ...
    try:
        ...
        if p_width and (not p_width.IsReadOnly):
            try:
                ...
                width = 1.0
            if font == "Arial" and width <= ARIAL_NARROW_THRESHOLD and is_font_installed("Arial Narrow"):
                p_font.Set("Arial Narrow")
                p_width.Set(1.0)
                return (True, "Arial->Narrow")

        if font and is_font_installed(font):
            canon = canonicalize_case_only(font)
            if canon and canon != font and is_font_installed(canon):
                p_font.Set(canon)
...
def get_footprint(tt):
    ...
    try: bold = tt.get_Parameter(DB.BuiltInParameter.TEXT_STYLE_BOLD).AsInteger()
    except: bold = 0
    try: italic = tt.get_Parameter(DB.BuiltInParameter.TEXT_STYLE_ITALIC).AsInteger()
    except: italic = 0
    try: underline = tt.get_Parameter(DB.BuiltInParameter.TEXT_STYLE_UNDERLINE).AsInteger()
    except: underline = 0

    # En tu Revit: 1=Transparent, 0=Opaque
    try: bg = tt.get_Parameter(DB.BuiltInParameter.TEXT_BACKGROUND).AsInteger()
    except: bg = 0
...
def find_existing_office_type(size_mm, require_name=None):
    """
    Returns a TextNoteType that matches the office standard properties:
      - Font = Arial
      - Width = 1.0
      - Background = Opaque (0 in your Revit)
      - Size matches size_mm (within SNAP_TOL_MM)
    If require_name is provided, also matches exact name (normalized).
    """
    ...
    for tt in DB.FilteredElementCollector(doc).OfClass(DB.TextNoteType):
        try:
            ...
            if not _is_close(_get_width(tt), 1.0, tol=0.01):
                continue
            if _get_bg_int(tt) != 0:  # 0 = Opaque (measured)
                continue
            smm = _get_size_mm(tt)
            if smm is None:
                continue
...
# --- Compatibility upgrade: strict_clean support (IronPython safe) ---
# Older versions defined find_existing_office_type without strict_clean kw.
# We redefine it here with a superset signature and strict checks when requested.
...
def find_existing_office_type(size_mm, require_name=None, strict_clean=True):
    ...
    Superset finder. If strict_clean=True, requires a "clean" office type:
      ...
      - Show Border = 0
      - Arrowhead default A30 (read-only check)
...
def standard_name(size_mm):
    # Office "standard" names have NO "STANDARD" prefix
...
def find_base_type_for_duplication(types):
    for tt in types:
        ...
        try:
            ...
            f = ""
        if norm(f).lower() == "arial":
            return tt
    for tt in types:
        n = get_type_name(tt)
        if not is_excluded_name(n):
            return tt
    return None
...
def create_office_standard_types():
    ...
    if not base:
        failed.append(("ALL", "", "No base TextNoteType found"))
    ...
    for size_mm in OFFICE_STANDARD_SIZES_MM:
        ...
        # 1) If a STRICT clean office type already exists for this size (any name), do not create another.
        ...
        # 2) If legacy exists and is clean, try rename to new name (no prefix) if free.
        ...
        # 3) If new_name exists but is NOT clean, we still want a clean one:
        #    create a new clean type with a unique name (will get (2) if needed).
        if norm(new_name) in existing_names:
            office_skip_log.append((float(size_mm), "name-exists-but-not-clean", new_name))
            # We'll still proceed to create, but name must be unique.
            new_name = ensure_unique_name(new_name, existing_names)

        legacy_name = sanitize_name("STANDARD {} {:.2f}mm".format(OFFICE_STANDARD_FONT, float(size_mm)))

        # If a compliant office type already exists (even with legacy STANDARD prefix), do NOT create another.
        if find_existing_office_type(size_mm, require_name=new_name) is not None:
            continue
        if find_existing_office_type(size_mm, require_name=legacy_name) is not None:
            # Optionally rename legacy -> new (no prefix), if name is free
            try:
                if norm(legacy_name) in existing_names and norm(new_name) not in existing_names:
                    # rename legacy type to new name (no param changes)
                    tt_legacy = None
                    for tt in types:
                        if norm(get_type_name(tt)) == norm(legacy_name):
                            tt_legacy = tt
                            break
                    if tt_legacy:
                        set_clr_name(tt_legacy, new_name)
                        existing_names.discard(norm(legacy_name))
                        existing_names.add(norm(new_name))
                continue
            except:
        ...
        # Also skip if any compliant office type exists regardless of name
        if find_existing_office_type(size_mm, require_name=None) is not None:
            continue
        try:
            ...
            # STANDARD SOLO OPACO -> en tu Revit: 0 = Opaque
            ...
            failed.append((str(size_mm), new_name, str(ex)))
...
modo = forms.CommandSwitchWindow.show(
    ...
    message=(
        "pyMenvic | Text Style Standardizer (v2.4.4)\\n\\n"
...
created_standard = []
failed_standard = []
office_skip_log = []  # (size_mm, reason, matched_type_name)
failed_rename = []
log_size = []
...
def ejecutar(rename_only=False):
    ...
    for tt in types_after:
        ...
        try:
            ...
            count_rename_failed += 1
            failed_rename.append((tt.Id.IntegerValue, old_name, new_name, "rename-failed: {}".format(ex)))
    ...
    # also protect legacy names (if they still exist)
...
with revit.Transaction("pyMenvic | Text Style Standardizer (v2.4.4)"):
...
# Report
output.print_md("# pyMenvic | Text Style Standardizer (v2.4.4) — {}".format(modo))
...
output.print_md("- Text Size snapped (solo STANDARD mm): **{}**".format(count_size_snapped))
output.print_md("- Grupos merged: **{}**".format(count_groups_merged))
output.print_md("- Notas reasignadas: **{}**".format(count_notes_moved))
output.print_md("- Tipos duplicados eliminados: **{}**".format(count_types_deleted))
output.print_md("- Tipos renombrados: **{}**".format(count_renamed))
output.print_md("- Renombres fallidos: **{}**".format(count_rename_failed))
output.print_md("- Tipos purgados: **{}**".format(count_purged))
output.print_md("- Office standards creados: **{}**".format(len(created_standard)))
output.print_md("---")

if office_skip_log:
    output.print_md("## Office standards omitidos (muestra)")
