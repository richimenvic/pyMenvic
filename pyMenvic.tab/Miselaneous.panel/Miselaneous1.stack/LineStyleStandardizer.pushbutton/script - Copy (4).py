# -*- coding: utf-8 -*-
__title__  = "Line Style Standardizer"
...
Descripción
-----------
Herramienta para estandarizar Line Styles en Revit usando
propiedades y reglas de uso dentro del modelo.

Cambios clave respecto a la versión anterior
---------------------------------------------
1. Los SEEDS (standards base) siempre se protegen.
2. Los estilos canónicos por grupo solo se crean o protegen
   si el grupo tiene uso real en el modelo.
3. Los estilos con "(0)" que NO son seed:
   - no generan estilo canónico
   - se intentan eliminar del proyecto.

Estructura del script
---------------------
El archivo está organizado en bloques. Para modificar o
reemplazar partes del código busca:
...
def props_need_update(sub, weight, color_obj, pattern_id, gtype=DB.GraphicsStyleType.Projection):
    """True solo si alguna propiedad difiere de lo esperado."""
    try:
        current_fp = subcat_footprint(sub, gtype)
    except:
        current_fp = None
    target_fp = footprint_tuple(weight, color_obj, pattern_id)
    return current_fp != target_fp


def get_expected_seed_names():
    ...
    for w in sorted(WEIGHT_LABELS.keys()):
        for color_label, _color_obj, pattern_label, _pattern_id in seeds:
            names.append(canonical_name(w, color_label, pattern_label))
    return names
...
def seed_canonical_standards(line_cat, gtype, seed_names):
    ...
    for w in sorted(WEIGHT_LABELS.keys()):
        for color_label, color_obj, pattern_label, pattern_id in seeds:
            ...
            # SEED MODE seguro: reusar solo por nombre canónico (root o root (n))
            ...
            if canon_sub is None:
                ...
                if props_need_update(canon_sub, w, color_obj, pattern_id, gtype):
                    updated_local += 1
...
if SEED_ONLY:
    ...
    ts = DB.Transaction(doc, "pyMENVIC: Seed Canonical Line Styles")
    ts.Start()
    ...
    usage_after_seed = count_curve_usage()
    seed_usage_rows = []
    seed_with_use = 0
    seed_zero_use = 0

    for canon_name in get_expected_seed_names():
        sub = find_subcat_by_root(line_cat, canon_name)
        if not sub:
            seed_usage_rows.append((canon_name, 0, "MISSING"))
            seed_zero_use += 1
            continue

        gsid = get_subcat_gsid(sub, gtype)
        cnt = usage_after_seed.get(gsid, 0) if gsid is not None else 0
        status = "OK"
        if cnt > 0:
            seed_with_use += 1
        else:
            seed_zero_use += 1
        try:
            seed_usage_rows.append((sub.Name, cnt, status))
        except:
            seed_usage_rows.append((canon_name, cnt, status))
    ...
    output.print_md("CANONICAL (SEED) STYLES WITH USE: {0}".format(seed_with_use))
    output.print_md("CANONICAL (SEED) STYLES WITH 0 USE: {0}".format(seed_zero_use))
    output.print_md("")
    output.print_md("## Canonical Seed Styles Usage")
    output.print_md("| Line Style | Uses | Status |")
    output.print_md("|---|---:|---|")
    for row_name, row_cnt, row_status in seed_usage_rows:
        output.print_md("| {0} | {1} | {2} |".format(row_name, row_cnt, row_status))
...
t = DB.Transaction(doc, "pyMENVIC: Line Master (Property Based)")
t.Start()
