...
def find_exact_subcat_fresh(name):
    try:
        fresh_line_cat = doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_Lines)
    except:
        fresh_line_cat = None
    if not fresh_line_cat or not name:
        return None
    try:
        if fresh_line_cat.SubCategories.Contains(name):
            return fresh_line_cat.SubCategories.get_Item(name)
    except:
        pass
    try:
        for sc in list(fresh_line_cat.SubCategories):
            try:
                if (sc.Name or "") == name:
                    return sc
            except:
...
def run_seed_only_from_diagnostic():
    ...
    for w in sorted(WEIGHT_LABELS.keys()):
        for color_label, color_obj, pattern_label, pattern_id in seeds:
            canon_name = canonical_name(w, color_label, pattern_label)
            before = find_exact_subcat_fresh(canon_name)
            existed_before = True if before else False
            reason = ""
            after = None

            t = DB.Transaction(doc, "MENVIC: Seed " + canon_name)
            try:
                t.Start()
                target = before
                if not target:
                    fresh_line_cat = doc.Settings.Categories.get_Item(DB.BuiltInCategory.OST_Lines)
                    target = doc.Settings.Categories.NewSubcategory(fresh_line_cat, canon_name)
                    doc.Regenerate()
                apply_props(target, w, color_obj, pattern_id, gtype)
                doc.Regenerate()
                status = t.Commit()
                doc.Regenerate()
            ...
            after = find_exact_subcat_fresh(canon_name)
            if after:
                if existed_before:
                    updated += 1
                else:
                    created += 1
                try:
                    gsid = get_subcat_gsid(after, gtype)
                    cnt = usage_after.get(gsid, 0) if gsid is not None else 0
                except:
                    cnt = 0
                rows.append((canon_name, cnt, "OK" if cnt > 0 else "ZERO USE"))
            else:
                rows.append((canon_name, 0, "FAILED CREATE" if not existed_before else "MISSING"))
                fail_details.append((canon_name, reason or "Not found after commit"))

    with_use = len([r for r in rows if r[2] == "OK"])
    zero_use = len([r for r in rows if r[2] == "ZERO USE"])
    missing = len([r for r in rows if r[2] in ("FAILED CREATE", "MISSING")])
    ...
    output.print_md("CANONICAL (SEED) STYLES MISSING/FAILED: {0}".format(missing))
    output.print_md("")
